...
export async function writeSessionBundle(
  ...
  await mkdir(join(dir, "dom"), { recursive: true });

  await writeFile(join(dir, "meta.json"), JSON.stringify(bundle.meta, null, 2) + "\n", "utf8");
  await writeFile(join(dir, "actions.jsonl"), toJsonl(bundle.actions), "utf8");

  const redactedNetwork = bundle.network.map(redactNetworkEntry);
  await writeFile(join(dir, "network.jsonl"), toJsonl(redactedNetwork), "utf8");
  await writeFile(join(dir, "index.json"), JSON.stringify(bundle.index, null, 2) + "\n", "utf8");

  for (const keyframe of bundle.keyframes) {
    await writeFile(join(dir, "keyframes", keyframe.name), keyframe.bytes);
  }

  for (const snapshot of bundle.domSnapshots) {
    await writeFile(
      join(dir, "dom", snapshot.name),
...
export async function readSessionBundle(dir: string): Promise<SessionBundleReadOutput> {
  ...
  const actionsRaw = await readFile(join(dir, "actions.jsonl"), "utf8");
  const actions = parseJsonl<ActionEntry>(actionsRaw);

  const networkRaw = await readFile(join(dir, "network.jsonl"), "utf8");
  const network = parseJsonl<NetworkEntry>(networkRaw);

  const indexRaw = await readFile(join(dir, "index.json"), "utf8");
...
export async function assertValidBundleDir(dir: string): Promise<void> {
  ...
  if (!(await fileExists(metaPath))) {
    throw new Error(`Invalid session bundle: missing meta.json in ${dir}`);
  }
  if (!(await fileExists(actionsPath))) {
    throw new Error(`Invalid session bundle: missing actions.jsonl in ${dir}`);
  }
}
